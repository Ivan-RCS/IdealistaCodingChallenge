...
public class AdvertisementDeserializer implements JsonDeserializer<Advertisement>
{
    ...
    public Advertisement deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException
    {
        ...
        Optional.ofNullable(obj.get("gardenSize")).ifPresent(o -> advertisement.setGardenSize(Optional.of(o.getAsInt())));
        ...
        try
        {
            if (idPictures.size() > 0)
            {
                ...
                InputStream inputStream = classLoader.getResourceAsStream(Constants.JSON_FILE_PICTURES);
                ...
                {
                    pictures.stream().filter(p -> p.getId().equals(jsonElement.getAsLong())).findFirst().ifPresent(advertisement.getPictures()::add);
                }
            }
